# 📘 Interfaces Digitais: Erros e Acertos sob a Ótica da UX

## 📌 Apresentação do Projeto
Este projeto consiste no desenvolvimento de uma cartilha educativa voltada à análise e comparação de interfaces digitais, destacando exemplos de boas e más práticas em UX/UI.

## 🎯 Objetivo da Cartilha
Comparar interfaces bem projetadas e problemáticas, demonstrando na prática a aplicação de boas práticas em projetos de UX/UI, com foco em clareza, usabilidade e experiência do usuário.

## 👥 Público-alvo
- Estudantes de Design UX/UI  
- Estudantes de Sistemas para Internet  
- Pessoas interessadas em design digital e experiência do usuário  

## 🧠 Contextualização do Tema
Com a evolução de aplicativos e sites, as interfaces digitais passaram a incorporar princípios da psicologia e da interação humano–computador, visando experiências mais eficientes, acessíveis e intuitivas. Nesse contexto, a cartilha auxilia na identificação e compreensão de interfaces bem ou mal projetadas.

## 👨‍💻👩‍💻 Equipe
- Hellen Vitória Santana de Souza
- Nereu Vítor Pereira Lima  
- Emanoel Victor Rocha Bezerra 
